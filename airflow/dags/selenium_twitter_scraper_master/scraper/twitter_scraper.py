...
- Twitter is experiencing unusual activity"""
        ...
        while self.scroller.scrolling:
            try:
                ...
                for card in self.tweet_cards[-15:]:
                    if(firstscroll and self.is_comment):
                        firstscroll = False
                        continue
                    try:
                        tweet_id = str(card)
                            ...
                            if tweet:
                                if not tweet.error and tweet.tweet is not None:
                                    if not tweet.is_ad:
                                            ...
                                            if(not self.is_comment):
                                                currTweet = json.dumps(tweet.tweet, indent= 4).encode("utf-8") ###########
                                                self.producer.send("raw_tweet", currTweet)
                                            else:
                                                print(f"comment: {tweet.tweet_id} of {self.mail}")
                                            added_tweets += 1
    ...
    def save_to_csv(self): #chaaaaaaaaaaaaaaaange 
        ...
        data = [
        {
            ...
            "Emojis": tweet["Emojis"],
            "Profile Image": tweet["Profile Image"],
            "Tweet Link": tweet["Tweet Link"],
            "Tweet ID": f"tweet_id:{tweet["Tweet ID"]}"
        } for tweet in self.data]

        if self.scraper_details["poster_details"]:
            for d in range(len(data)):
                data[d]["User ID"] = self.data[d]["Tweeter ID"]
                data[d]["Following Count"] = self.data[d]["Following Count"]
                data[d]["Followers Count"] = self.data[d]["Followers Count"] 
